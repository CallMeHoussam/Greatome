...
{
    if (std::ifstream(".system/first_time"))
    {
        // If the file exists, skip showing the terms
        return;
    }
    ...
    {
        size_t pos = line.find("Disclaimer");
        if (pos != std::string::npos)
        {
            line.replace(pos, 13, RED BOLD "⚠️  Disclaimer⚠️" RESET);
        }

        pos = line.find("ExamMocker ");
        if (pos != std::string::npos)
        {
            line.replace(pos, 13,  BOLD "ExamMocker" RESET);
        }

        pos = line.find("1337 School");
        if (pos != std::string::npos)
        {
            line.replace(pos, 14, YELLOW BOLD "1337 School" RESET);
        }

        pos = line.find("42 Network");
        if (pos != std::string::npos)
        {
            line.replace(pos, 10, YELLOW BOLD "42 Network" RESET);
        }

        pos = line.find("responsibly");
        if (pos != std::string::npos)
        {
            line.replace(pos, 14, PURPLE BOLD " responsibly " RESET);
        }

        pos = line.find("there are no shortcuts to success");
        if (pos != std::string::npos)
        {
            line.replace(pos, 36, RED BOLD"there are no shortcuts to success." RESET);
        }

        pos = line.find("may not align");
        if (pos != std::string::npos)
        {
            line.replace(pos, 16, RED "may not align" RESET);
        }

        pos = line.find("responsible for your own exam preparation and results ");
        if (pos != std::string::npos)
        {
            line.replace(pos, 54, RED BOLD "responsible for your own exam preparation and results" RESET);
        }

        pos = line.find("**ExamShell**");
        if (pos != std::string::npos)
        {
            line.replace(pos, 13, DARK_GREEN BOLD"ExamShell" RESET);
        }

        pos = line.find("**\"agree\"**");
        if (pos != std::string::npos)
        {
            line.replace(pos, 11, DARK_GREEN BOLD"\"agree\"" RESET);
        }

        // Replace other "**" with terminal bold escape sequences
        while ((pos = line.find("**")) != std::string::npos)
        {
            line.replace(pos, 2, BOLD);
            if ((pos = line.find("**", pos)) != std::string::npos)
                line.replace(pos, 2, RESET);
        }
    ...
    {
        std::ofstream acceptFile(".system/acceptCGV");
        acceptFile << "1";
        acceptFile.close();

        // Create a file to indicate the user has accepted the terms
        std::ofstream firstTimeFile(".system/first_time");
        firstTimeFile << "1";
        firstTimeFile.close();

        std::cout << "Thanks for accepting these rules, and good luck for your exam!" << std::endl;
        sleep(2);
    }
}
